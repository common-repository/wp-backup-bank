...
/**
 * This file is used for google task runner.
 *
 * @author  Tech Banker
 * @package wp-backup-bank/lib/Google/Task
 * @version 3.0.1
 */
...
class Google_Task_Runner {
	 ...
	 * @return boolean
	 */
	public function canAttmpt() {// @codingStandardsIgnoreLine.
	...
	public function run() {
		while ( $this->attempt() ) {
			try {
				...
				$allowedRetries = $exception->allowedRetries();// @codingStandardsIgnoreLine.

				if ( ! $this->canAttmpt() || ! $allowedRetries ) {// @codingStandardsIgnoreLine.
					throw $exception;
				}

				if ( $allowedRetries > 0 ) {// @codingStandardsIgnoreLine.
					$this->maxAttempts = min(// @codingStandardsIgnoreLine.
						$this->maxAttempts, $this->attempts + $allowedRetries// @codingStandardsIgnoreLine.
					);
				}
			}
		}
