...
abstract class Google_Logger_Abstract {
	...
	*/
	...
	/**
	 * The urrent log format.
	 *
	 * @var string $logFormat The current log format
	 */
	protected $logFormat = self::DEFAULT_LOG_FORMAT;// @codingStandardsIgnoreLine.
	/**
	 * The current date format.
	 *
	 * @var string $dateFormat The current date format
	 */
	protected $dateFormat = self::DEFAULT_DATE_FORMAT;// @codingStandardsIgnoreLine.
	/**
	 * The new lines are allowed.
	 *
	 * @var boolean $allowNewLines If newlines are allowed
	 */
	protected $allowNewLines = false;// @codingStandardsIgnoreLine.
	/**
	...
	public function __construct( Google_Client $client ) {
		...
		$this->logFormat = $format ? $format : self::DEFAULT_LOG_FORMAT;// @codingStandardsIgnoreLine.

		$format           = $client->getClassConfig( 'Google_Logger_Abstract', 'date_format' );
		$this->dateFormat = $format ? $format : self::DEFAULT_DATE_FORMAT;// @codingStandardsIgnoreLine.

		$this->allowNewLines = (bool) $client->getClassConfig(// @codingStandardsIgnoreLine.
	 ...
	 * @param integer $level .
	 */
	public function setLevel( $level ) {// @codingStandardsIgnoreLine.
	 ...
	 * @return boolean
	 */
	public function shouldHandle( $level ) {// @codingStandardsIgnoreLine.
	...
	public function log( $level, $message, array $context = array() ) {
		...
		$levelName = is_int( $level ) ? array_search( $level, self::$levels ) : $level;// @codingStandardsIgnoreLine.
		$message   = $this->interpolate(
			array(
				'message'  => $message,
				'context'  => $context,
				'level'    => strtoupper( $levelName ),// @codingStandardsIgnoreLine.
	...
	protected function interpolate( array $variables = array() ) {
		$template = $this->logFormat;// @codingStandardsIgnoreLine.
	 ...
	 * @param array $context The log context.
	 */
	protected function reverseJsonInContext( array &$context ) {// @codingStandardsIgnoreLine.
		if ( ! $context ) {
			return;
		}

		foreach ( $context as $key => $val ) {
			if ( ! $val || ! is_string( $val ) || ! ( '{' == $val[0] || '[' == $val[0] ) ) {// WPCS: Loose comparison ok.
				continue;
			}

			$json = @wp_json_decode( $val );// @codingStandardsIgnoreLine.
	...
	protected function export( $value ) {
		if ( is_string( $value ) ) {
			if ( $this->allowNewLines ) {// @codingStandardsIgnoreLine.
				return $value;
			}
		...
		if ( $value instanceof DateTime ) {
			return $value->format( $this->dateFormat );// @codingStandardsIgnoreLine.
		}
		...
		if ( version_compare( PHP_VERSION, '5.4.0', '>=' ) ) {
			...
			return @wp_json_encode( $value, $options );// @codingStandardsIgnoreLine.
		}

		return str_replace( '\\/', '/', @wp_json_encode( $value ) );// @codingStandardsIgnoreLine.
	}
	/**
	 ...
	 */
	protected function normalizeLevel( $level ) {// @codingStandardsIgnoreLine.
		if ( is_int( $level ) && array_search( $level, self::$levels ) !== false ) {// @codingStandardsIgnoreLine.
			return $level;
		}
