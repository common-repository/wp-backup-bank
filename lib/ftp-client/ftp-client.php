...
/**
 * This file is used for the FTP and SSL-FTP client for PHP
 *
 * @author  Tech Banker
...
class FtpClient {
	...
	public function setphplimit( $memory = null, $time_limit = 0, $ignore_user_abort = true ) {
		if ( null !== $memory ) {
			ini_set( 'memory_limit', $memory ); // @codingStandardsIgnoreLine.
		}
	...
	public function connect( $host, $ssl = false, $port = 21, $timeout = 90 ) {
		if ( $ssl ) {
			$this->conn = @$this->ftp->ssl_connect( $host, $port, $timeout ); // @codingStandardsIgnoreLine
		} else {
			$this->conn = @$this->ftp->connect( $host, $port, $timeout ); // @codingStandardsIgnoreLine
		}
	 ...
	 * @return resource
	 */
	public function getConnection() { // @codingStandardsIgnoreLine
		return $this->conn;
	}
	/**
	 * Get the wrapper
	 ...
	 * @return FtpWrapper
	 */
	public function getWrapper() { // @codingStandardsIgnoreLine
		return $this->ftp;
	}
	/**
	 ...
	 * @return int
	 */
	public function modifiedtime( $remoteFile, $format = null ) { // @codingStandardsIgnoreLine.
		$time = $this->ftp->mdtm( $remoteFile ); // @codingStandardsIgnoreLine.
		if ( -1 !== $time && null !== $format ) {
	 ...
	 */
	public function up() {
		$result = @$this->ftp->cdup(); // @codingStandardsIgnoreLine.
		if ( false === $result ) {
	...
	public function nlist( $directory = '.', $recursive = false, $filter = 'sort' ) {
		...
		// if it's the current .
		if ( false !== ( $kdot = array_search( '.', $files ) ) ) { // @codingStandardsIgnoreLine.
			unset( $files[ $kdot ] );
		}
	...
	public function mkdir( $directory, $recursive = false ) {
		...
		foreach ( $parts as $part ) {
			if ( ! @$this->ftp->chdir( $part ) ) { // @codingStandardsIgnoreLine.
	...
	public function cleandir( $directory ) {
		if ( ! $files = $this->nlist( $directory ) ) { // @codingStandardsIgnoreLine.
	...
	public function remove( $path, $recursive = false ) {
		try {
			if ( @$this->ftp->delete( $path ) // @codingStandardsIgnoreLine.
			|| ( $this->isDir( $path ) && @ $this->rmdir( $path, $recursive ) ) ) { // @codingStandardsIgnoreLine.
				return true;
			}
			return false;
		} catch ( \Exception $e ) {
	 ...
	 * @throws FtpException .
	 */
	public function isDir( $directory ) { // @codingStandardsIgnoreLine.
		$pwd = $this->ftp->pwd();
		...
		if ( @$this->ftp->chdir( $directory ) ) { // @codingStandardsIgnoreLine.
			$this->ftp->chdir( $pwd );
			return true;
		}
	 ...
	 * @return bool
	 */
	public function isEmpty( $directory ) { // @codingStandardsIgnoreLine.
	...
	public function count( $directory = '.', $type = null, $recursive = true ) {
		...
		$count = 0;
		foreach ( $items as $item ) {
			if ( null === $type || $item['type'] == $type ) { // WPCS:loose comparison ok.
				$count++;
			}
		}
		return $count;
	...
	public function putfromstring( $remote_file, $content ) {
		$handle = fopen( 'php://temp', 'w' ); // @codingStandardsIgnoreLine.

		fwrite( $handle, $content ); // @codingStandardsIgnoreLine.
		rewind( $handle );
	...
	public function putfrompath( $local_file ) {
		...
		$handle      = fopen( $local_file, 'r' ); // @codingStandardsIgnoreLine.
	...
	public function putall( $source_directory, $target_directory, $mode = FTP_BINARY ) {
		...
		while ( $file = $d->read() ) { // @codingStandardsIgnoreLine.
			// to prevent an infinite loop .
			if ( '.' != $file && '..' != $file ) { // WPCS:loose comparison ok.
				// do the following if it is a directory .
				if ( is_dir( $source_directory . '/' . $file ) ) {
					if ( ! @$this->ftp->chdir( $target_directory . '/' . $file ) ) { // @codingStandardsIgnoreLine.
					...
					// recursive part .
					$this->putAll(
						$source_directory . '/' . $file, $target_directory . '/' . $file, $mode
					);
				} else {
					// put the files .
					$this->ftp->put(
						$target_directory . '/' . $file, $source_directory . '/' . $file, $mode
					);
				}
			}
		}
	...
	public function rawlist( $directory = '.', $recursive = false ) {
		...
		if ( false == $recursive ) { // WPCS: loose comparison ok.
			foreach ( $list as $path => $item ) {
				$chunks = preg_split( '/\s+/', $item );
				if ( empty( $chunks[8] ) || '.' == $chunks[8] || '..' == $chunks[8] ) { // WPCS:loose comparison ok.
					continue;
				}

				$path = $directory . '/' . $chunks[8];
				if ( isset( $chunks[9] ) ) {
					$nbChunks = count( $chunks ); // @codingStandardsIgnoreLine.
					for ( $i = 9; $i < $nbChunks; $i++ ) { // @codingStandardsIgnoreLine.
						$path .= ' ' . $chunks[ $i ];
					}
				}

				if ( substr( $path, 0, 2 ) == './' ) { // WPCS:loose comparison ok.
					$path = substr( $path, 2 );
				}
		...
		foreach ( $list as $item ) {
			...
			if ( ! $len
			|| ( '.' == $item[ $len - 1 ] && '' == $item[ $len - 2 ]  // WPCS:loose comparison ok.

			|| '.' == $item[ $len - 1 ] && '.' == $item[ $len - 2 ] && '' == $item[ $len - 3 ] ) // WPCS:loose comparison ok.
			) {
				continue;
			}
			...
			if ( 'd' == $item[0] ) { // WPCS:loose comparison ok.
	...
	public function parserawlist( array $rawlist ) {
		...
		foreach ( $rawlist as $key => $child ) {
			...
			if ( isset( $chunks[8] ) && ( '.' == $chunks[8] || '..' == $chunks[8] ) ) { // WPCS:loose comparison ok.
				continue;
			}
			...
			if ( count( $chunks ) === 1 ) {
				...
				if ( $len && ':' == $chunks[0][ $len - 1 ] ) { // WPCS:loose comparison ok.
			...
			if ( 'link' == $item['type'] ) { // WPCS:loose comparison ok.
